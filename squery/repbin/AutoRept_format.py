...
class AutoRept(object):
    ...
    def __init__(self, input_file='', cnv_data='', infor='', cosmic='', sample_id='', outdir='', template_dir='', report = ''):
        ...
        self.hash_genes[u'肺癌至尊版'] =  ['ABL1', 'AKT1', 'ALK', 'APC', 'ARID1A', 'ATM', 'BRAF', 'BRCA1', 'BRCA2',
            'CDK6', 'CDKN2A', 'CTNNB1', 'DDR2', 'EGFR', 'ERBB2', 'ERBB3', 'ERBB4', 'ESR1', 'FBXW7',
            'FGFR1', 'FGFR2', 'FGFR3', 'FLT3', 'GNA11', 'GNAQ', 'HNF1A', 'HRAS', 'IDH1', 'IDH2',
            'JAK1', 'JAK2', 'KDR', 'KIT', 'KRAS', 'MAP2K1', 'MET', 'MLH1', 'MPL', 'MTOR', 'NF1',
            'NOTCH1', 'NPM1', 'NRAS', 'NTRK3', 'PDGFRA', 'PIK3CA', 'PTEN', 'RAF1', 'RET', 'RICTOR',
        ...
        self.hash_genes[u'EGFR基础版'] =  ['EGFR']
    ...
    def load_result_gene(self, input_file):
        ...
        with open(input_file, 'r') as row_values:
            for i, row_value in enumerate(row_values):
                ...
                # pass gibberish data
                if len(list_row) < 29 or i == 0:continue
                Chr = list_row[1].strip()
                start = list_row[2].strip()
                ref = list_row[4].strip()
                alt = list_row[5].strip()
                cur_gene = list_row[6].strip()
                anno_type = list_row[11].strip()
                cur_cosm = list_row[17].strip()
                cur_c_anno = list_row[9].strip()
                cur_mutation = list_row[10].strip()
                rs_num = list_row[26].strip()
                cur_vf = float(list_row[12].strip())
                cur_dp = list_row[13].strip()
                cur_mutation_num = list_row[14].strip()
                pos = "%s:%s" %(Chr, start)
    ...
    def __parse_type(self,text):
        s1 = re.search(ur'(肺癌|肿瘤化疗|EGFR|肠癌)', text)
